...
def get_posed_images(provider, mps_traj, stream_id):
    ...
    # EQ patch: subsample ACROSS the whole recording (stride) rather than taking
    # the first 300 consecutive frames, so we get proper multi-view baseline for
    # reconstruction + mesh instead of a ~10-30s clip.
    target = 300
    stride = max(1, num_data // target)
    cnt = 0
    for idx in range(start_idx, num_data, stride):
...
def main(vrs_file: Path, mps_data_dir: Path, output_dir: Path, force_delete=True, start_end_idx=None):
    ...
    # EQ patch: the saved images are the SPHERICAL reprojection (dst_calib2),
    # which is exactly equidistant r=f*theta (verified). So the COLMAP camera
    # must be OPENCV_FISHEYE with all distortion coeffs = 0 (pure equidistant),
    # NOT SIMPLE_PINHOLE. Principal from the spherical calibration.
    princ = dst_calib2.get_principal_point()
    cx, cy = float(princ[0]), float(princ[1])
    colmap_cam = pycolmap.Camera(
        model="OPENCV_FISHEYE",
        width=tgt_img_size, height=tgt_img_size,
        params=np.array([tgt_focal, tgt_focal, cx, cy, 0.0, 0.0, 0.0, 0.0]),
        camera_id=0,
    )
    ...
    for idx, (image_data, extrinsics_device) in tqdm(enumerate(get_posed_images(provider, mps_traj, stream_id))):
        ...
        # EQ patch: colour from the equidistant (saved) image, not the linear one.
        pcd_colors[pcd_is_visible_this_frame] = undistorted_array[pcd_2d_xy[:, 1], pcd_2d_xy[:, 0]]
