...
MultiviewNCC(const torch::Tensor& depths,
    ...
    if (normals.size(0) != depths.size(0) || uvs.size(0) != depths.size(0)) {
        AT_ERROR("depths/normals/uvs must agree on num_points");
    }
...
MultiviewNCCBackward(const torch::Tensor& depths,
    ...
    if (patch_radius > 5) {
        AT_ERROR("multiview_ncc_backward: patch_radius > 5 exceeds the "
                 "kernel's per-thread stash (BWD_MAX_RADIUS in "
                 "multiview_ncc_impl.cu) -- raise BWD_MAX_RADIUS and "
                 "recompile, or use the finite-difference backward");
    }
    if (normals.size(0) != depths.size(0) || uvs.size(0) != depths.size(0) ||
        grad_ncc.size(0) != depths.size(0)) {
        AT_ERROR("depths/normals/uvs/grad_ncc must agree on num_points");
    }

    const int P     = depths.size(0);
    auto float_opts = depths.options().dtype(torch::kFloat32);

    torch::Tensor grad_depths  = torch::zeros({P}, float_opts);
    ...
    CHECK_CUDA(multiview_ncc_backward_launcher(
                   P,
                   depths.contiguous().data_ptr<float>(),
                   normals.contiguous().data_ptr<float>(),
                   uvs.contiguous().data_ptr<int>(),
                   ...
                   patch_radius,
                   precise,
                   grad_ncc.contiguous().data_ptr<float>(),
                   grad_depths.contiguous().data_ptr<float>(),
                   grad_normals.contiguous().data_ptr<float>()),
               debug);

    return std::make_tuple(grad_depths, grad_normals);
}
